...
extern "C"
JNIEXPORT jfloatArray JNICALL
Java_com_example_ys_orbtest_VideoModeSLAM_CVTest(JNIEnv *env, jobject instance, jlong matAddr) {
...
//                cv::circle(*pMat, projectedPoints[0], 2, cv::Scalar(0, 0, 250), 1, 8);
            ...
            LOGE("ORB-SLAM完成：pose判断为空 ");
    }

    switch(SLAM->GetTrackingState()) {

        case -1: {cv::putText(*pMat, "SYSTEM NOT READY", cv::Point(0,400), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(255,0,0),2);         LOGE("当前的SLAM状态 : SYSTEM NOT READY");}break;
        case 0:  {cv::putText(*pMat, "NO IMAGES YET", cv::Point(0,400), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(255,0,0),2); LOGE("当前的SLAM状态 : NO IMAGES YET");}break;
        case 1:  {cv::putText(*pMat, "SLAM NOT INITIALIZED", cv::Point(0,400), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(255,0,0),2);LOGE("当前的SLAM状态 : SLAM NOT INITIALIZED"); }break;
        case 2:  {cv::putText(*pMat, "SLAM ON", cv::Point(0,400), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(0,255,0),2); LOGE("当前的SLAM状态 : SLAM ON");break;}
        case 3:  {cv::putText(*pMat, "SLAM LOST", cv::Point(0,400), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(255,0,0), 2); LOGE("当前的SLAM状态 : SLAM LOST");break;}
        default:break;
    }
...
Java_com_example_ys_orbtest_VideoModeSLAM_ShutDown(JNIEnv *env, jobject instance){
     SLAM->Shutdown();
}